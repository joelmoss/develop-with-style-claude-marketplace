...
and use that script as the default build/run path for every subsequent
`/build-and-run-macos-app`, `/test-macos-app`, or manual run request.
...
1. Discover the project shape.
   ...
   - If the user explicitly asks for version control, run `git init` at the project/workspace root. Never run `git init` inside a nested subdirectory when the current workspace already belongs to a parent repo.
...
3. Create or update `script/build_and_run.sh`.
   ...
   - Use `references/run-button-bootstrap.md` as the canonical source for the
     script shape. Do not fork a second authoritative snippet in another skill
     or command.
...
- `references/run-button-bootstrap.md`: canonical `build_and_run.sh` script shape for SwiftPM CLI tools and SwiftPM/AppKit/SwiftUI GUI apps.
